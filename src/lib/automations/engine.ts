...
async function runStep(step: AutomationStep, args: ExecuteArgs): Promise<string> {
  ...
  switch (step.step_type) {
    ...
    case 'create_deal': {
      ...
      // Deals no longer carry a flat value/currency (migration 036) — the
      // value derives from line items (deal_products). An automation-created
      // deal starts with no line items; products can be added later in the UI.
